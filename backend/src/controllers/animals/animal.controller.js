...
animalCtrl.getAllAnimals = async (req, res) => {
  try {
    const allanimal = await db.query(
      "SELECT animals.name AS nameAnimal, animals.location"+
      ",animals.description, animals.photo AS animalPhoto, animals.sound AS animalSound, ctlg_type_specie.name AS typeSpecie"+
      ", ctlg_scientific_name_animal.scientific_name AS scientificName "+
